  ...
  run: function () {
    ...
    if (argv.start) {
      ...
      prompt.get({
        ...
        that.fetchWrapper(argv.start, 'deploy', 'https://github.com/phodal-archive/echeveria-deploy/archive/master.zip');
        that.fetchWrapper(argv.start, 'content', 'https://github.com/phodal-archive/echeveria-content/archive/master.zip');

        if (withMobileApp) {
          console.log("fetch mobile .. ");
          that.fetchWrapper(argv.start, 'mobile', 'https://github.com/phodal-archive/echeveria-mobile/archive/master.zip');
        }
        if (withEditor) {
          console.log("fetch editor .. ");
          that.fetchWrapper(argv.start, 'editor', 'https://github.com/phodal/echeveria-editor/archive/master.zip');
        }
      });
    }
  },
